...
def test_orders_permissions():
    ...
    if resp.ok:
        ...
        print(f'  [FAIL] Admin 登录失败: {resp.json().get("error")}')
        return
    ...
    if not operator_id:
        ...
        if resp.ok:
            ...
            print(f'  [OK] 创建 operator 成功 (id={operator_id})')
        else:
            print(f'  [FAIL] 创建 operator 失败: {resp.json().get("error")}')
    ...
    if not viewer_id:
        ...
        if resp.ok:
            ...
            print(f'  [OK] 创建 viewer 成功 (id={viewer_id})')
        else:
            print(f'  [FAIL] 创建 viewer 失败: {resp.json().get("error")}')
    ...
    if resp.ok:
        print('  [OK] Operator 登录成功')
    else:
        print(f'  [FAIL] Operator 登录失败: {resp.json().get("error")}')
        return
    ...
    if resp.ok:
        print('  [OK] Viewer 登录成功')
    else:
        print(f'  [FAIL] Viewer 登录失败: {resp.json().get("error")}')
        return
    ...
    if resp.ok:
        data = resp.json()
        print(f'  [OK] Admin 查看全部订单: {data["total"]} 条')
    else:
        print(f'  [FAIL] Admin 查看全部订单失败: {resp.status_code}')
    
    # Operator 请求 scope=all 应该只能看到自己的
    resp = operator_session.get(f'{BASE_URL}/api/orders?scope=all')
    if resp.ok:
        data = resp.json()
        print(f'  [OK] Operator 查看订单(请求all): {data["total"]} 条 (应该只看到自己的)')
    else:
        print(f'  [FAIL] Operator 查看订单失败: {resp.status_code}')
    
    # Viewer 请求 scope=all 应该被强制改为 own
    resp = viewer_session.get(f'{BASE_URL}/api/orders?scope=all')
    if resp.ok:
        data = resp.json()
        print(f'  [OK] Viewer 查看订单(请求all): {data["total"]} 条 (应该只看到自己的或0)')
    else:
        print(f'  [FAIL] Viewer 查看订单失败: {resp.status_code}')
    ...
    if resp.ok:
        ...
        print(f'  [OK] Admin 创建订单成功: id={admin_order_id}')
    else:
        print(f'  [FAIL] Admin 创建订单失败: {resp.json().get("error")}')
        admin_order_id = None
    ...
    if resp.ok:
        ...
        print(f'  [OK] Operator 创建订单成功: id={operator_order_id}')
    else:
        print(f'  [FAIL] Operator 创建订单失败: {resp.json().get("error")}')
        operator_order_id = None
    
    # Viewer 创建订单 (应该失败)
    resp = viewer_session.post(f'{BASE_URL}/api/orders', json={**test_order, 'job_no': 'TEST003'})
    if resp.status_code == 403:
        print('  [OK] Viewer 创建订单被拒绝 (403 Forbidden) - 符合预期')
    else:
        print(f'  [FAIL] Viewer 创建订单: {resp.status_code} (应该返回403)')
    ...
    if admin_order_id:
        ...
        if resp.ok:
            print(f'  [OK] Admin 编辑自己的订单成功')
        else:
            print(f'  [FAIL] Admin 编辑自己的订单失败: {resp.json().get("error")}')
        ...
        if operator_order_id:
            resp = admin_session.put(f'{BASE_URL}/api/orders/{operator_order_id}', 
                                    json={'consignee': 'Updated by Admin'})
            if resp.ok:
                print('  [OK] Admin 编辑 Operator 的订单成功 (admin有权限)')
            else:
                print(f'  [FAIL] Admin 编辑 Operator 的订单失败: {resp.json().get("error")}')
    ...
    if operator_order_id:
        ...
        if resp.ok:
            print(f'  [OK] Operator 编辑自己的订单成功')
        else:
            print(f'  [FAIL] Operator 编辑自己的订单失败: {resp.json().get("error")}')
        ...
        if admin_order_id:
            ...
            if resp.status_code == 403:
                print('  [OK] Operator 编辑 Admin 的订单被拒绝 (403 Forbidden) - 符合预期')
            else:
                print(f'  [FAIL] Operator 编辑 Admin 的订单: {resp.status_code} (应该返回403)')
    ...
    if admin_order_id:
        ...
        if resp.status_code == 403:
            print('  [OK] Viewer 编辑订单被拒绝 (403 Forbidden) - 符合预期')
        else:
            print(f'  [FAIL] Viewer 编辑订单: {resp.status_code} (应该返回403)')
    ...
    if operator_order_id:
        ...
        if resp.ok:
            print('  [OK] Operator 删除自己的订单成功')
        else:
            print(f'  [FAIL] Operator 删除自己的订单失败: {resp.json().get("error")}')
    
    # Viewer 删除订单 (应该失败)
    resp = viewer_session.delete(f'{BASE_URL}/api/orders/{operator_order_id or admin_order_id or 1}')
    if resp.status_code == 403:
        print('  [OK] Viewer 删除订单被拒绝 (403 Forbidden) - 符合预期')
    else:
        print(f'  [FAIL] Viewer 删除订单: {resp.status_code} (应该返回403)')
