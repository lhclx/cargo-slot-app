...
def add_order():
    ...
    # SO号唯一性检查
    so_no = str(order.get('so_no', '')).strip()
    if so_no and any(str(o.get('so_no', '')) == so_no for o in data):
        return jsonify({'error': f'SO号 {so_no} 已存在,不能重复'}), 400
...
def update_order(order_id):
    ...
    for i, o in enumerate(data):
        if o['id'] == order_id:
            ...
            upd = request.get_json()
            # SO号唯一性检查(不能改成别人已有的SO号)
            new_so = str(upd.get('so_no', '')).strip()
            if new_so and any(str(o.get('so_no', '')) == new_so and o['id'] != order_id for o in data):
                return jsonify({'error': f'SO号 {new_so} 已存在,不能重复'}), 400
